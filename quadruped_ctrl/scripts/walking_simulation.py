...
def init_simulator():
    ...
    high_performance_mode = p.addUserDebugParameter(
    ...
    if terrain == "plane":
        ...
        gazebo_world_parser.parseWorld(
            p, filepath="worlds/racetrack_day.world")
...
def camera_update():
    ...
    pointcloud_publisher = rospy.Publisher(
        "/generated_pc", PointCloud2, queue_size=10)
    ...
    while not rospy.is_shutdown():
        ...
        cameraTargetPosition = (
            numpy.mat(T3_)*numpy.array([[0], [0], [1], [1]]))[0:3]

        q = pyquaternion.Quaternion(matrix=T3_)
        cameraQuat = [q[1], q[2], q[3], q[0]]

        robot_tf.sendTransform(
            cubePos, cubeOrn, rospy.Time.now(), "robot", "world")
        robot_tf.sendTransform(cameraEyePosition, cameraQuat,
                               rospy.Time.now(), "cam", "world")
        robot_tf.sendTransform(cameraTargetPosition,
                               cubeOrn, rospy.Time.now(), "tar", "world")
        ...
        pub_image.data = numpy.asarray(
            grey.convert('L')).reshape([1, -1]).tolist()[0]
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if(high_performance_flag < p.readUserDebugParameter(high_performance_mode)):
            high_performance_flag = p.readUserDebugParameter(
...
if __name__ == '__main__':
    ...
    rospy.loginfo("lateralFriction = " + str(lateralFriction) +
                  " spinningFriction = " + str(spinningFriction))
    ...
    pub_zebra_ctrl = rospy.Publisher(
        "/ZebraJointControl", ZebraJointControl, queue_size=1)
